...
def dump_pcm_cavity_coords(pcmobj, tag=""):
    ...
    for i, xyz in enumerate(coords):
        print(f"{i:5d}  {xyz[0]: .8f}  {xyz[1]: .8f}  {xyz[2]: .8f}")

def fresh_dm(mol, base_mf, dm0=None):
    mf = base_mf.copy()
    mf.reset(mol)
    if getattr(mf, "with_solvent", None) is not None:
        mf.with_solvent.reset(mol)
...
def hess_elec(pcmobj, base_mf, eps=5e-3, verbose=None):
    '''
    ...
    mol = pmol.copy()
    print("initial grid")
    dump_pcm_cavity_coords(pcmobj, tag="initial grid")
    print("geometry of initial structure in Bohr")
    coords = mol.atom_coords(unit='Bohr')
    print(coords)
    de = numpy.zeros([mol.natm, mol.natm, 3, 3])
    dm_ref = base_mf.make_rdm1(ao_repr=True)   # based density matrix. putpose: improving the SCF convergence
    #print(dm_ref)

    log.warn("Using finite difference scheme for electrostatic contribution")
    print(f"differencial range : ", eps, "[Bohr]")
    mol.verbose = 0

    for ia in range(mol.natm):
        for ix in range(3):
            disp = numpy.zeros_like(coords)
            disp[ia,ix] = eps
            #print("disp")
            #print(disp)

            mol_p  = mol.copy(); mol_p.set_geom_(coords + disp, unit = 'Bohr')
            print("-"*60)
            print(f"geometry of finite difference +delta",ia, ix)
            print()
            print(mol_p.atom_coords(unit='Bohr'))

            pcm_p  = copy.copy(pcmobj); pcm_p.mol = mol_p # new coodinate
            dump_pcm_cavity_coords(pcm_p, tag=f"+eps: atom {ia} {ix}")
            print("-"*60)

            dm_ref = fresh_dm(mol_p, base_mf, dm0=dm_ref) # SCF loop & update new density matrix (dm)
            pcm_p.reset(mol_p)
            print("Result after PCM calculation")
            _e, _  = pcm_p._get_vind(dm_ref) # PCM calculation
            g_p    = (grad_nuc(pcm_p, dm_ref) + grad_solver(pcm_p, dm_ref) + grad_qv(pcm_p, dm_ref))

            mol_m  = mol.copy(); mol_m.set_geom_(coords - disp, unit = 'Bohr')
            print("-"*60)
            print(f"geometry of finite difference -delta",ia, ix)
            print()
            print(mol_m.atom_coords(unit='Bohr'))
            
            pcm_m  = copy.copy(pcmobj); pcm_m.mol = mol_m
            dump_pcm_cavity_coords(pcm_m, tag=f"-eps: atom {ia} {ix}")
            print("-"*60)

            dm_ref = fresh_dm(mol_m, base_mf, dm0=dm_ref) # SCF loop & update new density matrix (dm)
            pcm_m.reset(mol_m)
            _e, _  = pcm_m._get_vind(dm_ref)
            g_m    = (grad_nuc(pcm_m, dm_ref) + grad_solver(pcm_m, dm_ref) + grad_qv(pcm_m, dm_ref))

            de[ia, :, ix] = (g_p - g_m)/(2.0*eps)

    de = 0.5 * (de + numpy.transpose(de, (1, 0, 3, 2)))
    asym = de - np.transpose(de, (1, 0, 3, 2))
    log.debug('Hess asym = %.2e', np.abs(asym).max())
    log.timer_debug1('solvent Hessian (FD)', *t0)

    pcmobj.reset(pmol)
    return de

def fd_grad_vmat(pcmobj, base_mf, atmlst=None, eps=5e-03, verbose=None):
    log = logger.new_logger(pcmobj, verbose)
    t0  = (logger.process_clock(), logger.perf_counter())
    pmol   = base_mf.mol.copy()
    mol    = pmol.copy()
    coords = mol.atom_coords(unit='Bohr')
    ...
    if atmlst is None:
        atmlst = range(mol.natm)
    nao = mol.nao
    vgrad = np.empty((len(atmlst), 3, nao, nao))
    dm_ref = base_mf.make_rdm1(ao_repr=True)
    log.warn("Using finite-difference scheme for electrostatic potential")
    for i0, ia in enumerate(atmlst):
        for ix in range(3):
            ...
            mol_p = mol.copy(); mol_p.set_geom_(coords + disp, unit='Bohr')
            pcm_p = copy.copy(pcmobj); pcm_p.mol = mol_p
            dm_ref = fresh_dm(mol_p, base_mf, dm0=dm_ref)
            pcm_p.reset(mol_p)
            _e, v_plus = pcm_p._get_vind(dm_ref)
            #dump_pcm_cavity_coords(pcm_p, tag=f"+eps: atom {ia} {ix}")

            mol_m = mol.copy(); mol_m.set_geom_(coords - disp, unit='Bohr')
            pcm_m = copy.copy(pcmobj); pcm_m.mol = mol_m
            dm_ref = fresh_dm(mol_m, base_mf, dm0=dm_ref)
            pcm_m.reset(mol_m)
            _e, v_minus = pcm_m._get_vind(dm_ref)
            #dump_pcm_cavity_coords(pcm_p, tag=f"+eps: atom {ia} {ix}")

            vgrad[i0, ix] = (v_plus - v_minus) / (2.0 * eps)

    log.timer_debug1('computing solvent grad veff (FD)', *t0)
    pcmobj.reset(pmol)
    return vgrad

from pyscf.solvent import pcm as pyscf_pcm
def clone_pcm_keep_grid(pcmobj: pyscf_pcm.PCM, mol_new: gto.Mole ) -> pyscf_pcm.PCM:
    if getattr(pcmobj, 'surface', None) is None or 'grid_coords' not in pcmobj.surface:
        pcmobj.build()
    new_pcm         = copy.copy(pcmobj)
    new_pcm.mol     = mol_new # replace molecule
    new_pcm.surface = pcmobj.surface # same dict -> same grid_coords, etc...
    return new_pcm

def dump_pcm_safe(pcmobj: pyscf_pcm.PCM, tag: str = "") -> None:
    """Print **grid_coords**; build PCM first if the surface is absent."""
    if getattr(pcmobj, 'surface', None) is None or 'grid_coords' not in pcmobj.surface:
        pcmobj.build()
    coords = pcmobj.surface['grid_coords'] 
    print(f"----------- {tag} (Ngrid={len(coords)}) -----------")
...
def _copy_scf_settings(src_mf: scf.hf.SCF, dst_mf: scf.hf.SCF) -> None:
    attrs = ('conv_tol', 'conv_tol_grad', 'max_cycle', 'xc', 'level_shift', 'damp',
             'diis', 'diis_start_cycle', 'verbose', 'stdout')
    for key in attrs:
        if hasattr(src_mf, key):
            setattr(dst_mf, key, getattr(src_mf, key))

def fresh_dm_fixedgrid(
        mol_new: gto.Mole, 
        base_mf: scf.hf.SCF, 
        *, 
        dm0: numpy.ndarray | None = None,
        keep_grid: bool = True,
):
    pcm_ref = getattr(base_mf, 'with_solvent', None)

    if pcm_ref is None or not keep_grid:
        # --- fall back to original behaviour ------------------------------
        mf = base_mf.copy(); mf.reset(mol_new)
        if pcm_ref is not None:
            mf.with_solvent.reset(mol_new)
            mf.kernel(dm0=dm0, dump_chk=False)
            return mf.make_rdm1(ao_repr=True)

    # ---------------------------------------------------------------------
    # build new SCF object with a *frozen* cavity grid ----------------------
    # ---------------------------------------------------------------------
    mf = base_mf.copy(); mf.reset()
    mf.with_solvent = clone_pcm_keep_grid(pcm_ref, mol_new)

    #dump_pcm_safe(mf.with_solvent, tag="fresh_dm_fixedgrid")

    mf.kernel(dm0=dm0, dump_chk=False)
    return mf.make_rdm1(ao_repr=True)
    

def hess_elec_fixedgrid(
    pcmobj: pyscf_pcm.PCM,
    base_mf: scf.hf.SCF,
    *,
    eps: float = 1.89e-3,
    verbose: int | None = None,
):
    """Finite‑difference Hessian of the electrostatic contribution with a
    *fixed* cavity surface grid.
    """
    log = logger.new_logger(pcmobj, verbose)
    t0 = (logger.process_clock(), logger.perf_counter())
    ...
    dm_ref0 = base_mf.make_rdm1(ao_repr=True) # initial structure

    log.warn('Using finite‑difference scheme for electrostatic Hessian '
             '(surface grid frozen)')
    log.info('eps = %g Bohr', eps)
    print("-"*60)
    print("initial grid")
    dump_pcm_safe(pcmobj, tag="initial coordinate")
    print("-"*60)
    ...
    for ia in range(mol.natm):
        for ix in range(3):
            ...
            mol_p = mol.copy(); mol_p.set_geom_(coords0 + disp, unit='Bohr')
            pcm_p = clone_pcm_keep_grid(pcmobj, mol_p) 
            dump_pcm_safe(pcm_p, tag=f"+eps: atom {ia} {ix}")
            #dm_ref = dm_ref0 # not SCF
            dm_ref = fresh_dm_fixedgrid(mol_p, base_mf, dm0=dm_ref0) # SCF loop
            _e, _ = pcm_p._get_vind(dm_ref)
            g_p = (grad_nuc(pcm_p, dm_ref) +
                   grad_solver(pcm_p, dm_ref) +
                   grad_qv(pcm_p, dm_ref))

            mol_m = mol.copy(); mol_m.set_geom_(coords0 - disp, unit='Bohr')
            pcm_m = clone_pcm_keep_grid(pcmobj, mol_m)
            dump_pcm_safe(pcm_m, tag=f"-eps: atom {ia} {ix}")
            dm_ref = fresh_dm_fixedgrid(mol_m, base_mf, dm0=dm_ref) # SCF loop
            _e, _ = pcm_m._get_vind(dm_ref)
            g_m = (grad_nuc(pcm_m, dm_ref) +
                   grad_solver(pcm_m, dm_ref) +
                   grad_qv(pcm_m, dm_ref))
    ...
    log.timer_debug1('solvent Hessian (FD, fixed grid)', *t0)

    pcmobj.reset(pmol)  # restore original PCM object
    return de

def fd_grad_vmat_fixedgrid(
    pcmobj: pyscf_pcm.PCM,
    base_mf: scf.hf.SCF,
    *,
    atmlst=None,
    ...
    """Finite‑difference derivative of the PCM potential matrix with a
    *fixed* cavity surface grid.
    """
    log = logger.new_logger(pcmobj, verbose)
    t0 = (logger.process_clock(), logger.perf_counter())
    ...
    dm_ref0 = base_mf.make_rdm1(ao_repr=True)
    ...
    for i0, ia in enumerate(atmlst):
        for ix in range(3):
            ...
            mol_p = mol.copy(); mol_p.set_geom_(coords0 + disp, unit='Bohr')
            pcm_p = clone_pcm_keep_grid(pcmobj, mol_p)
            #dump_pcm_safe(pcm_p, tag=f"+eps check: atom {ia} {ix}")  #bdebug
            #dm_ref = dm_ref0 # not SCF
            dm_ref = fresh_dm_fixedgrid(mol_p, base_mf, dm0=dm_ref0)
            _e, v_plus = pcm_p._get_vind(dm_ref)

            mol_m = mol.copy(); mol_m.set_geom_(coords0 - disp, unit='Bohr')
            pcm_m = clone_pcm_keep_grid(pcmobj, mol_m)
            dm_ref = fresh_dm_fixedgrid(mol_m, base_mf, dm0=dm_ref)
            _e, v_minus = pcm_m._get_vind(dm_ref)

            vgrad[i0, ix] = (v_plus - v_minus) / (2.0 * eps)

    log.timer_debug1('PCM grad veff (FD, fixed grid)', *t0)
    return vgrad
...
class WithSolventHess:
    ...
    def kernel(self, *args, dm=None, atmlst=None, **kwargs):
        ...
        self.de_solvent = hess_elec(self.base.with_solvent, self.base, eps=5e-3, verbose=self.verbose)
    ...
    def make_h1(self, mo_coeff, mo_occ, chkfile=None, atmlst=None, verbose=None):
        ...
        if isinstance(self.base, scf.hf.RHF):
            #dm = self.base.make_rdm1(ao_repr=True)
            dv = fd_grad_vmat(self.base.with_solvent, self.base, eps=5e-3, atmlst=atmlst, verbose=verbose)
            for i0, ia in enumerate(atmlst):
                h1ao[i0] += dv[i0]
            return h1ao
            ...
            #dm = self.base.make_rdm1(ao_repr=True)
            #dm = dm[0] + dm[1]
            dva = fd_grad_vmat(self.base.with_solvent, self.base, atmlst=atmlst, verbose=verbose)
            dvb = fd_grad_vmat(self.base.with_solvent, self.base, atmlst=atmlst, verbose=verbose)
            for i0, ia in enumerate(atmlst):
                h1aoa[i0] += dva[i0]
                h1aob[i0] += dvb[i0]
            return h1aoa, h1aob
    ...
    #def kernel(self, *args, dm=None, atmlst=None, **kwargs):
        ...
        eq_solv = self.base.with_solvent.equilibrium_solvation
        self.base.with_solvent.equilibrium_solvation = True

        self.de_solvent = hess_elec_fixedgrid(
            self.base.with_solvent, self.base,
            eps=1.89e-3, verbose=self.verbose
        )
        self.de_solute = super().kernel(*args, **kwargs)
        self.de = self.de_solute + self.de_solvent

        self.base.with_solvent.equilibrium_solvation = eq_solv
        return self.de

    #def make_h1(self, mo_coeff, mo_occ, chkfile=None, atmlst=None, verbose=None):
        """Add the solvent potential derivatives to the parent H1 matrix."""
        if atmlst is None:
            atmlst = range(self.mol.natm)
        h1ao = super().make_h1(mo_coeff, mo_occ, atmlst=atmlst, verbose=verbose)

        if isinstance(self.base, scf.hf.RHF):
            dv = fd_grad_vmat_fixedgrid(
                self.base.with_solvent, self.base,
            ...
            h1aoa, h1aob = h1ao  # unpack parent contribution
            dva = fd_grad_vmat_fixedgrid(
                self.base.with_solvent, self.base,
                atmlst=atmlst, verbose=verbose
            )
            dvb = fd_grad_vmat_fixedgrid(
                self.base.with_solvent, self.base,
                atmlst=atmlst, verbose=verbose
            )
            for i0, ia in enumerate(atmlst):
            ...
            raise NotImplementedError('Base object is not RHF nor UHF')
